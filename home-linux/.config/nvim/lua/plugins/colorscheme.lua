...
--
-- Noctalia's templates render lua/matugen.lua (machine-local); when it exists
-- a "noctalia" scheme joins the cycle and becomes the default. Disable the
-- template in noctalia.toml and delete that file for static Kanagawa.
--
...
local function apply(name)
  ...
  if name == "noctalia" then
    -- base16 paints highlights directly; there is no :colorscheme to call.
...
-- Registered unconditionally: the default SIGUSR1 disposition KILLS nvim, so
-- every instance needs a handler even on Kanagawa.
