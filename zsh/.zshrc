...
# ~/.zsh_secrets is never tracked by git. See the README's Secrets section.
...
# Same directory-jump convention as cc; use codex directly for a plain prompt.
cx() {
  if [[ $# -gt 0 && $1 != -* ]]; then
    z "$1" || return
    shift
...
# Private-repo extras, not shared publicly.
